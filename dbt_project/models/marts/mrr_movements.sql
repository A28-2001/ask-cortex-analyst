...
with_running_totals as (
    select
        ...
        new_customers,
        churned_customers,
        -- Starting MRR = cumulative net MRR up to but not including this month
        coalesce(
            sum(net_new_mrr) over (order by month rows between unbounded preceding and 1 preceding),
            0
        )                                                                              as starting_mrr,
...
final as (
    select
        ...
        new_customers,
        churned_customers,
        -- Net Revenue Retention: (ending MRR from existing customers) / starting MRR
        case
            when starting_mrr > 0
